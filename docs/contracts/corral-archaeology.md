...
| --- | --- | --- | --- | --- | --- |
| A1 | Typed Herdr socket RPC/event reconciliation | 6dfba22 · [`src/adapters/herdr.rs`](https://github.com/jirathip-dev/corral/blob/6dfba2249c86c3906b025b3098879ab241b691a2/src/adapters/herdr.rs) (event-first newline-delimited JSON-RPC over the per-user Unix socket; `events.subscribe` connections become push-only; request/response work opens a fresh short-lived connection; replay-convergent subscribe; trusted freshness watchdog; `DEFAULT_SOCKET`) · tests exist under `tests/` at the same commit | Apache-2.0 OR MIT | **ADAPT** | Herdr-socket wire knowledge is the starting point for the herdr adapter spec (child #7), but herdr-fleet must re-derive and re-verify every protocol fact against live Herdr + upstream docs — Corral code is evidence, not the contract. Review must verify: protocol facts re-verified, no adapter code copied wholesale, live-socket conformance tests in #7. |
