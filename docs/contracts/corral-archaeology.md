...
- **ADAPT** — take the artifact's shape/wire knowledge as a starting point
  and re-derive it for canter's own contract, with re-verification.
- **LEARN** — extract the discipline/pattern into canter's specs and
...
Upstream's read-only trajectory therefore corroborates two canter
